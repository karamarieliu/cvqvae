...
with torch.no_grad():
    for tr in range(trjs):
        ...
        latents.append(encoding_indices_x.detach().cpu().numpy().squeeze())
        ctxts.append(encoding_indices_c.detach().cpu().numpy().squeeze())

np.save(save_dir+'/bco_xlatents.npy',latents)
np.save(save_dir+'/bco_clatents.npy',ctxts)
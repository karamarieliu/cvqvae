...
data = np.load(data_dir+"/bcov5_0.npy")[:50*30]
...
with torch.no_grad():
    for tr in range(trjs):
        ...
        vec_x = model(x,vec_only=True)
        vec_c = model(c,vec_only=True)
        latents.append(vec_x.detach().cpu().numpy().squeeze().reshape(length,64,16,16))
        ctxts.append(vec_c.detach().cpu().numpy().squeeze().reshape(length,64,16,16))
